define(['application', 'dataService'], function (app) {
    app.register.directive('noeEjareSelect', ['dataService', function (service) {
        return {
            restrict: 'E',
            transclude: true,
            templateUrl: "/App/template/noe-ejare-select.html",
            scope: {
                filter: "=",
                text: "=?",
                eid: "=",
            ...
            link: function (scope, element, attrs) {
                ...
                scope.init = function () {
                    ...
                    service.getAll('/app/api/NoeEjare/GetAllByOrganId').then(function (data) {
                ...
                scope.submitNode = function () {
                    ...
                    service.addEntity("/app/api/NoeEjare/Add", scope.node).then(function (node) {
                        scope.callingSaveNode = false;
                        var tree = scope.tree;
                        var parent = tree.selected;
                        var find = findAndReplace(scope.nodes, node.ID, node);
                        if (find) tree.editNode(tree.selected, node, node.Title, "", "person");
                        else {
                            tree.addNode(parent, node, node.Title, "", "person");
