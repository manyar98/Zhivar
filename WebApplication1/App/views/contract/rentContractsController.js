define(['application', 'dataService', 'digitGroup', 'number', 'dx', 'roweditor', 'helper', 'gridHelper', 'messageService'], function (app) {
    app.register.controller('rentContractsController', ['$scope', 'dataService', '$location', '$stateParams', '$compile', '$state', '$rootScope', 'messageService',
        function ($scope, dataService, $location, $stateParams, $compile, $state, $rootScope, messageService) {
            ...
            $rootScope.applicationModule = " قراردادهای اجاره گرفته شده";
            $scope.applicationDescription = "در این صفحه امکان جستجو، اضافه و ویرایش قراردادهای اجاره گرفته شده وجود دارد.";
            ...
            $scope.init = function () {
                ...
                if ($scope.type === 0) $rootScope.pageTitle("قراردادهای اجاره به طرف حساب");
                else if ($scope.type === 1) $rootScope.pageTitle("قراردادهای اجاره از طرف حساب");
 

                var lookup = { dataSource: [{ key: 0, val: "ذخیره موقت" }, { key: 1, val: "ارسال شده" }, { key: 4, val: "تایید شده" }], valueExpr: 'key', displayExpr: 'val' };
                ...
                grid = dxGrid({
                    ...
                    columns: [
                        ...
                        {
                            caption: 'عملیات', dataField: '', width: 80, printVisible: false, cellTemplate: function (cellElement, cellInfo) {
            ...
            $scope.editInvoice = function (id) {
                $state.go('newRentContract', { id: id });
            ...
            $scope.startInvoice = function (id, status, title) {
                ...
                $scope.model.Code = 'RentContract';
                $scope.model.ID = id;
                // var ID = this.dataItem($(e.currentTarget).closest("tr")).ID;
                $scope.model.OrganizationID = 22;//this.dataItem($(e.currentTarget).closest("tr")).OrganizationID;
                // var InstanceTitle = " قرارداد " + title;//this.dataItem($(e.currentTarget).closest("tr")).DaroukhanehNam;
