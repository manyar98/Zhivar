define(['application', 'combo', 'scrollbar', 'dx', 'roweditor', 'helper', 'gridHelper', 'editBank', 'editCash', 'displayNumbers', 'decimalPointZero', 'keyboardFilter', 'digitGroup', 'datePicker', 'number'], function (app) {
    app.register.controller('viewContractController', ['$scope', '$rootScope', '$stateParams', '$location', '$compile', '$state',
        function ($scope, $rootScope, $stateParams, $location, $compile, $state) {
            ...
            $scope.deleteInvoice = function () {
                ...
                if ($scope.payments.length > 0) {
                    ...
                    tip1 += "نکته: با حذف این قرارداد، دریافت ها و پرداخت های ثبت شده برای این قرارداد نیز حذف می گردد.";
                    tip1 += "</div>";
                }
                ...
                questionbox({
                    content: "آیا از حذف این قرارداد مطمئن هستید؟" + tip1 + tip2,
            ...
            $scope.setViewInvoicePageTitle = function () {
                if ($scope.invoice.InvoiceType === 0) $rootScope.pageTitle("قرارداد" + ($scope.invoice.Contact ? " " + $scope.invoice.Contact.Name : "") + " - شماره قرارداد: " + $scope.invoice.Number);
            ...
            $scope.print = function (reset) {
                if (reset)
                    ...
                    $scope.invoiceSettings = {
                        ...
                        purchaseInvoiceTitle: "قرارداد خرید",
                        receiveReceiptTitle: "رسید دریافت وجه / چک",
                        rowPerPage: "18",
                        saleDraftInvoiceTitle: "پیش قرارداد",
            ...
            $scope.printReceiptDialog = function () {
                ...
                var strForWhat = "بابت قرارداد شماره " + $scope.invoice.Number;
            ...
            $scope.openSendInvoiceByEmailDialog = function () {
                ...
                $scope.emailSubject = "قرارداد شماره " + $scope.invoice.Number + " از طرف "
                    + $rootScope.currentBusiness.Name + " برای " + $scope.invoice.Contact.Name;
                $scope.emailText = "سلام " + $scope.invoice.Contact.Name + "\n";
                $scope.emailText += "قرارداد صادر شده برای شما به شماره " + $scope.invoice.Number + " و مبلغ "
                    + $scope.money($scope.invoice.Payable) + " " + $scope.getCurrency() + " را از ضمیمه ایمیل دانلود کنید.";
                $scope.emailText += "\n\n" + "تاریخ سررسید این قرارداد " + $scope.invoice.DisplayDueDate + " می باشد.";
            ...
            $scope.showPublicLink = function () {
                if (!$scope.invoiceSettings.onlineInvoiceEnabled) {
                    alertbox({ content: "ارسال قرارداد آنلاین فعال نیست. برای فعالسازی به منوی [تنظیمات / تنظیمات قرارداد آنلاین] مراجعه کنید." });
                    return;
                }
                if ($scope.calling)
                    return;
            ...
            $scope.copyPublicLink = function () {
                ...
                try {
                    ...
                    if (successful)
                        DevExpress.ui.notify("لینک قرارداد آنلاین کپی شد.", "success", 2000);
                    else
            ...
            $scope.emailPublicLink = function () {
                ...
                callws(DefaultUrl.MainWebService + "EmailOnlineInvoiceLink", { invoiceId: $scope.invoice.Id, email: $scope.contactEmail })
                    .success(function (data) {
                        $scope.calling = false;
                        DevExpress.ui.notify("لینک قرارداد آنلاین ایمیل شد.", "success", 3000);
                        linkPopup.hide();
                        applyScope($scope);
                    }).fail(function (error) {
                        $scope.calling = false;
