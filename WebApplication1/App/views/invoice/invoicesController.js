define(['application', 'dataService', 'digitGroup', 'number', 'dx', 'roweditor', 'helper', 'gridHelper', 'messageService'], function (app) {
    app.register.controller('invoicesController', ['$scope','dataService', '$location', '$stateParams', '$compile', '$state', '$rootScope', 'messageService',
...
//        function (  $window) {
            ...
            $rootScope.applicationModule = "فاکتور";
            $scope.applicationDescription = "در این صفحه امکان جستجو، اضافه و ویرایش فاکتور وجود دارد.";
    ...
    $scope.init = function () {
        ...
        else if (show && show === "rentTo") {
            $scope.type = 5;
            $scope.workflowCode = "";
        }
        else if (show && show === "rentFrom") {
            $scope.type = 6;
            $scope.workflowCode = "";
        }
        else {
        ...
        else if ($scope.type === 5) $rootScope.pageTitle("فاکتورهای اجاره");
        else if ($scope.type === 6) $rootScope.pageTitle("فاکتورهای اجاره از طرف حساب");
        ...
        grid = dxGrid({
			...
			{ caption: 'سررسید', dataField: 'DisplayDueDate', width: 100, printWidth: 3, hidingPriority: 4 },
			{ caption: 'مبلغ', dataField: 'Payable', dataType: "number", format: "#", width: 100, printWidth: 3, printFormat: "#" },
			{ caption: $scope.type === 0 ? 'دریافت شده' : 'پرداخت شده', dataField: 'Paid', dataType: "number", format: "#", width: 100, printWidth: 3, printFormat: "#", hidingPriority: 3 },
			{ caption: 'باقیمانده', dataField: 'Rest', dataType: "number", format: "#", width: 100, printWidth: 3, printFormat: "#", hidingPriority: 2 },
            {
                caption: 'وضعیت',
			    dataField: 'Status',
			    width: 100,
			...
			//{ caption: '', dataField: 'IsContract', width: 60, hidingPriority: 0 },
			{
			    caption: 'عملیات', dataField: '', width: 70, printVisible: false, cellTemplate: function (cellElement, cellInfo) {
                    if (cellInfo.data.Status === 0)
                    {
                        var html = "<a class='text-info p-l-15' href='javascript:void(0);' onclick='angular.element(this).scope().editInvoice(" + cellInfo.data.ID + "," + cellInfo.data.IsContract + "," + ")'><span class='fa fa-pencil'></span></a>";
            ...
            },
            {
                caption: 'گردش کار', dataField: '', width: 80, printVisible: false, cellTemplate: function (cellElement, cellInfo) {
                    if (cellInfo.data.Status === 0) {
                        ...
                        var html = "<button type='button' class='k-button k-button - icontext' onclick='angular.element(this).scope().startInvoice(" + cellInfo.data.ID + "," + cellInfo.data.Status + "," + '"' + cellInfo.data.Contact.Title + '"' +")'><span class='fa fa-reply'></span>ارسال</button>";
                        
                        cellElement.html(html);
                    }

                    else {
                        html = "<button type='button' class='k-button k-button - icontext' onclick='angular.element(this).scope().startInvoice(" + cellInfo.data.ID + "," + cellInfo.data.Status + "," + '"' + cellInfo.data.Contact.Title + '"' +")'><span class='fa fa-reply'></span>ارسال</button>";
    ...
    $scope.getInvoices = function () {
        ...
        $.ajax({
            ...
            url: "/app/api/Invoice/GetInvoices",
    ...
    $scope.editInvoice = function (id , isContract) {
    ...
    $scope.startInvoice = function (id, status, title) {
       ...
       // var ID = this.dataItem($(e.currentTarget).closest("tr")).ID;
        ...
        var InstanceTitle = title;//this.dataItem($(e.currentTarget).closest("tr")).DaroukhanehNam;
