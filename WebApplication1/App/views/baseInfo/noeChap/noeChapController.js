define(['application', 'combo', 'scrollbar', 'helper', 'editNoeChap', 'editContact', 'nodeSelect', 'roweditor', 'gridHelper', 'dx','messageService'], function (app) {
    app.register.controller('noeChapController', ['$scope', '$rootScope', '$location', '$compile', '$state',
        function ($scope, $rootScope, $location, $compile, $state, messageService) {
            var grid;
            $scope.init = function () {
                $rootScope.pageTitle("نوع چاپ");
                $('#businessNav').show();
                $scope.type = "all";
                $scope.nodeId = 0;
                ...
                grid = dxGrid({
                    ...
                    columns: [
                        ...
                        //{ caption: 'گروه', dataField: 'GroupName', printWidth: 5, allowEditing: false },
                        {
                          
                            caption: 'عنوان نوع چاپ', printWidth: 7, dataField: 'Title', allowEditing: false, cellTemplate: function (cellElement, cellInfo) {
                                cellElement.html("<a class='text-info txt-bold' href='#itemCard/" + cellInfo.data.Id + "'>" + Hesabfa.farsiDigit(cellInfo.displayValue) + "</a>");
                            }
                        },
                        //{ caption: 'قیمت خرید', dataField: 'BuyPrice', dataType: 'number', format: '#', width: 120, printWidth: 2, printFormat: "#", allowEditing: false },
                        //{ caption: 'قیمت فروش', dataField: 'SellPrice', dataType: 'number', format: '#', width: 120, printWidth: 2, printFormat: "#", allowEditing: false },
                        //{ caption: 'موجودی', dataField: 'Stock', dataType: 'number', width: 80, printWidth: 2, allowEditing: false },
                        //{ caption: 'بارکد', dataField: 'Barcode', dataType: 'number', width: 110, printWidth: 3 },
                        {
            ...
            $scope.getItems = function () {
                ...
                $.ajax({
                    ...
                    url: "/app/api/NoeChap/GetAllByOrganId",
            ...
            $scope.deleteItems = function () {
                ...
                function deleteItems(items, startIndex, count, successCallback, errorCallback) {
                    ...
                    $.ajax({
                        ...
                        url: "/app/api/NoeChap/DeleteNoeChaps",
                        contentType: "application/json"
                    }).done(function (res) {
                        ...
                        {
                            messageService.warning("خطایی رخ داده است.");
                        }
                ...
                function startDelete(items, start, count) {
                    if (start >= items.length) {
                        ...
                        $scope.calling = false;
                        var strMessage = deletedCount > 1 ? "نوع چاپ ها " : "نوع چاپ ";
                ...
                if (items.length === 0) {
                    alertbox({ content: "ابتدا يك يا چند نوع چاپ را انتخاب كنيد" });
                    return;
                }
                questionbox({
                    content: "آيا از حذف نوع چاپ های انتخاب شده مطمئن هستيد؟",
            ...
            $scope.goToUpdateItemPrice = function () {
                $state.go('updateItemPrice');
            }
