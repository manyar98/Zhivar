define(['application', 'dataService'], function (app) {
    app.register.directive('nodeSelect', ['dataService', function (service) {
    return {
        restrict: 'E',
        transclude: true,
        templateUrl: "/App/template/node-select.html",
        scope: {
            filter: "=",
            text: "=?",
            eid: "=",
        ...
        link: function (scope, element, attrs) {
            ...
            scope.init = function () {
                ...
                $.ajax({
                    ...
                    url: "app/api/ItemGroup/GetAllByOrganId",
                    ...
                    for (var i = 0; i < nodes.length; i++) {
                        if (scope.isInTree(nodes[i]) == null) {
                            if (nodes[i].Parent == null) {
                                ...
                                var node = scope.tree.addNode(null, nodes[i], nodes[i].Name, "", image);
                                treeNodes.push(node);
            ...
            scope.editNode = function () {
                ...
                scope.inputNodeName = scope.node.Name;
                applyScope(scope);
            };
            scope.submitNode = function () {
                ...
                scope.node.Name = scope.inputNodeName;
                ...
                $.ajax({
                    type: "POST",
                    data: JSON.stringify(scope.node),
                    url: "/app/api/ItemGroup/Add",
                    contentType: "application/json"
                }).done(function (res) {
                    var node = res.data;
                    ...
                    if (find) tree.editNode(tree.selected, node, node.Name, "", "good");
                    else {
                        tree.addNode(parent, node, node.Name, "", "good");
