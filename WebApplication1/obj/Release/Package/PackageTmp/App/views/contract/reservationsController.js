define(['application', 'dataService', 'digitGroup', 'number', 'dx', 'roweditor', 'helper', 'gridHelper', 'messageService', 'editReservation', 'combo', 'scrollbar'], function (app) {
    app.register.controller('reservationsController', ['$scope', 'dataService', '$location', '$stateParams', '$compile', '$state', '$rootScope', 'messageService',
        function ($scope, dataService, $location, $stateParams, $compile, $state, $rootScope, messageService) {
            ...
            $rootScope.applicationModule = "رزروها";
            $scope.applicationDescription = "در این صفحه امکان جستجو، اضافه و ویرایش رزروها وجود دارد.";
            ...
            $scope.init = function () {
                ...
                if (status && status === 'draft' && invoiceId) {
                    $scope.alert = true;
                    $scope.alertMessage = "رزرو {2} ذخیره شد - {0} - مبلغ کل: {1}".
                    ...
                    $scope.alert = true;
                    $scope.alertMessage = "رزرو جهت تایید ثبت و ذخیره شد - {0} - مبلغ کل: {1}".
                    ...
                    $scope.alert = true;
                    $scope.alertMessage = "پرداخت به مبلغ {0} ثبت شد - {1} - سر رسید: {2} - پرداخت در {3} - مبلغ رزرو: {4} - باقیمانده: {5}".
                    ...
                    $scope.alert = true;
                    $scope.alertMessage = "رزرو شماره {0} از سیستم حذف شد.".
                ...
                if ($scope.type === 0) $rootScope.pageTitle("رزروهای فروش");
                else if ($scope.type === 1) $rootScope.pageTitle("رزروهای خرید");
                ...
                grid = dxGrid({
                    ...
                    columns: [
                        ...
                        { caption: 'تاریخ', dataField: 'DisplayRegisterDate', width: 80, printWidth: 3 },

                        { caption: 'مدت اعتبار', dataField: 'ValiditDuration', dataType: "number", format: "#", width: 80, printWidth: 3, printFormat: "#" },
                        { caption: 'رنگ', dataField: 'Color', dataType: "number", format: "#", width: 80, visible: false  },
                        //{
                        //    caption: 'وضعیت',
                        //    dataField: 'Status',
                        //    width: 100,
                        ...
                        //{ caption: 'شهر', dataField: 'City', width: 60, hidingPriority: 0 },
                        {
                            caption: 'عملیات', dataField: '', width: 60, printVisible: false, cellTemplate: function (cellElement, cellInfo) {
                                if (cellInfo.data.Status === 0) {
                                    ...
                                    html = "<a class='text-info p-l-15' href='javascript:void(0);' onclick='angular.element(this).scope().editInvoice(" + cellInfo.data.ID + ")'><span class='fa fa-pencil'></span></a>";
                                    html += "<a class='text-danger' href='javascript:void(0);' onclick='angular.element(this).scope().deleteInvoice()'><span class='fa fa-times'></span></a>";
                                    cellElement.html(html);
                                }
                        ...
                        //{
                        //    caption: 'گردش کار', dataField: '', width: 150, printVisible: false, cellTemplate: function (cellElement, cellInfo) {
                        //        if (cellInfo.data.Status === 0) {

                        //            var html = "<button type='button' class='k-button k-button - icontext' onclick='angular.element(this).scope().startInvoice(" + cellInfo.data.ID + "," + cellInfo.data.Status + ")'><span class='fa fa-reply'></span>لیست سازه ها </button>";
                        //            html += "<button type='button' class='k-button k-button - icontext' onclick='angular.element(this).scope().startWorkFlowGroupOfSaze(" + cellInfo.data.ID + "," + cellInfo.data.Status + "," + '"' + cellInfo.data.ContactTitle + '"' + ")'><span class='fa fa-reply'></span>ارسال </button>";
                    ...
                    onCellPrepared: function (e) {
                        if (e.rowType === "data" && e.data.Color === 1) 
                                e.cellElement.css("background", "#FF0008");
                        if (e.rowType === "data" && e.data.Color === 2)
                                e.cellElement.css("background", "#FF8700");
                        if (e.rowType === "data" && e.data.Color === 3)
                                e.cellElement.css("background", "#18C700");
            ...
            $scope.getInvoices = function () {
                ...
                $.ajax({
                    ...
                    url: "/app/api/Reservation/GetReservations",
            ...
            $scope.startInvoice = function (id, status, title) {
                ...
                $state.go('sazesOfContracts', { id: id });

                //if (status === 1) {
                //    DevExpress.ui.notify(" این رزرو قبلا ارسال شده است. ", "warning", 3000);
            ...
            $scope.startWorkFlowGroupOfSaze = function (id, status, title) {
                //delete after test
                // $state.go("contractProfile", { id: id });
                // $state.go("diff");
                ...
                $scope.model.InstanceTitle = title;
                var organizationID = 22;//this.dataItem($(e.currentTarget).closest("tr")).OrganizationID;


                dataService.addEntity('/app/api/Contract/startWorkFlowGroupOfSaze', $scope.model).then(function (data) {
            ...
            $scope.deleteInvoice = function () {
                ...
                if (items.length === 0) {
                    alertbox({ content: "هیچ رزروی جهت حذف انتخاب نشده است" });
                    return;
                }
                var s = items.length === 1 ? "رزرو" : "رزروهای";
                ...
                if (haveApproved) {
                    ...
                    //tip1 += "نکته: با حذف رزروهای تایید شده، دریافت ها و پرداخت های ثبت شده برای آنها نیز حذف می گردد.";
                    //tip1 += "</div>";
                    if ($scope.type === 1 || $scope.type === 2) {
                        tip2 = "<div class='bg-warning pd-5 small'>";
                        tip2 += "حذف رزرو خرید (یا برگشت از فروش) ممکن است باعث ایجاد موجودی منفی در کالاها شود";
                ...
                questionbox({
                    ...
                    onBtn1Click: function () {
                        ...
                        $.ajax({
                            ...
                            grid.refresh();
                            grid.clearSelection();
                            var strMessage = items.length > 1 ? "رزروهای " : "رزرو ";
            ...
            $scope.returnInvoice = function () {
                ...
                if (selectedItems.length === 0) {
                    alertbox({ content: "هیچ رزروی جهت مرجوع کردن انتخاب نشده است" });
                    return;
                }
                if (selectedItems.length > 1) {
                    alertbox({ content: "برای مرجوع کردن فقط یک رزرو را انتخاب کنید" });
                    return;
                }
                if (selectedItems[0].Returned) {
                    alertbox({ content: "این رزرو قبلاً مرجوع شده است." });
                    return;
                }
                questionbox({
                    content: "آیا از مرجوع کردن رزرو انتخاب شده مطمئن هستید؟",
                    onBtn1Click: function () {
                        if ($scope.type === 0)
            ...
            $scope.markAs = function (status) {
                ...
                var selectedItems = grid.getSelectedRowsData();
                if (selectedItems.length === 0) {
                    alertbox({ content: "هیچ رزروی انتخاب نشده است" });
                    return;
                }
                var s = selectedItems.length === 1 ? "رزرو" : "رزروهای";
                for (var i = 0; i < selectedItems.length; i++) {
                    if (selectedItems[i].Status < 2) {
                        alertbox({ content: "یکی از رزروهای انتخاب شده تایید نشده است و امکان تغییر وضعیت آن وجود ندارد." });
                        return;
                    }
                }
                if ($scope.calling) return;
            ...
            $scope.calculateInvoicesStatuses = function () {
                ...
                for (var i = 0; i < len; i++) {
                    if (invoices[i].Status === 2) draft++;
                    else if (invoices[i].Status === 3) awaitingApproval++;
                    else if (invoices[i].Status === 4) approved++;
                }
            ...
            $scope.printAddressLabelDialog = function () {
                ...
                messagebox({
                    ...
                    "<label><input type='checkbox' id='invoiceNumber' checked> شماره رزرو</label><br/>" +
                    "<label><input type='checkbox' id='invoiceNote'> افزودن توضیحات رزرو به برچسب</label>" +
                    "</div>" +
                    "</div><hr>" +
            ...
            $scope.printInvoices = function () {
                var gridSelectedItems = grid.getSelectedRowsData();
                if (gridSelectedItems.length === 0) {
                    alertbox({ content: "ابتدا یک یا چند رزرو را انتخاب کنید." });
                    return;
                }
                $("#printBtns").hide();
                $("#progressBarDiv").show();
                ...
                (function loop() {
                    callws(DefaultUrl.MainWebService + 'Load_Invoice_TransObj', { id: gridSelectedItems[i].Id })
                        .success(function (result) {
                            ...
                            var l = $scope.invoice.InvoiceItems.length; // محاسبه مالیات و تخفیف کل رزرو جهت نمایش
                            ...
                            $("#progressPdfTitle").text("در حال تهیه PDF... رزرو " + Hesabfa.farsiDigit(i + 1) + " از " + Hesabfa.farsiDigit(listCount) + " (" + Hesabfa.farsiDigit(progressValue) + '%)');
                            i++;
