define(['application', 'dataService', 'digitGroup', 'number', 'dx', 'roweditor', 'helper', 'gridHelper', 'messageService'], function (app) {
    app.register.controller('contractsController', ['$scope', 'dataService', '$location', '$stateParams', '$compile', '$state', '$rootScope', 'messageService',
        function ($scope, dataService, $location, $stateParams, $compile, $state, $rootScope, messageService) {
            ...
            $rootScope.applicationModule = "قراردادها";
            $scope.applicationDescription = "در این صفحه امکان جستجو، اضافه و ویرایش قراردادها وجود دارد.";
            ...
            $scope.init = function () {
                ...
                if (status && status === 'draft' && invoiceId) {
                    ...
                    $scope.alert = true;
                    $scope.alertMessage = "قرارداد جهت تایید ثبت و ذخیره شد - {0} - مبلغ کل: {1}".
                    ...
                    $scope.alert = true;
                    $scope.alertMessage = "پرداخت به مبلغ {0} ثبت شد - {1} - سر رسید: {2} - پرداخت در {3} - مبلغ قرارداد: {4} - باقیمانده: {5}".
                    ...
                    $scope.alert = true;
                    $scope.alertMessage = "قرارداد شماره {0} از سیستم حذف شد.".
                ...
                if ($scope.type === 0) $rootScope.pageTitle("قراردادهای فروش");
                else if ($scope.type === 1) $rootScope.pageTitle("قراردادهای خرید");
                ...
                grid = dxGrid({
                    ...
                    columns: [
                        ...
                        {
                            caption: 'عنوان قرارداد',
                            dataField: 'ContractTitle',
                            printWidth: 5,
                            cellTemplate: function (cellElement, cellInfo) {
                        ...
                        { caption: 'تاریخ', dataField: 'DisplayDate', width: 80, printWidth: 3 },

                        { caption: 'مبلغ', dataField: 'Payable', dataType: "number", format: "#", width: 80, printWidth: 3, printFormat: "#" },
                        ...
                        //{ caption: 'شهر', dataField: 'City', width: 60, hidingPriority: 0 },
                        {
                            caption: 'عملیات', dataField: '', width: 60, printVisible: false, cellTemplate: function (cellElement, cellInfo) {
                                if (cellInfo.data.Status === 0) {
                                    ...
                                    html = "<a class='text-info p-l-15' href='javascript:void(0);' onclick='angular.element(this).scope().viewInvoice(" + cellInfo.data.ID + ")'><span class='fa fa-pencil'></span></a>";
                                    html += "<a class='text-danger' href='javascript:void(0);' onclick='angular.element(this).scope().deleteInvoice()'><span class='fa fa-times'></span></a>";
                                    cellElement.html(html);
                                }
                        ...
                        {
                            caption: 'گردش کار', dataField: '', width: 200, printVisible: false, cellTemplate: function (cellElement, cellInfo) {
                                if (cellInfo.data.Status === 0) {
                                    ...
                                    html = "<button type='button' class='k-button k-button - icontext' onclick='angular.element(this).scope().startInvoice(" + cellInfo.data.ID + "," + cellInfo.data.Status + ")'><span class='fa fa-reply'></span> لیست سازه ها</button>";
                                    html += "<button type='button' class='k-button k-button - icontext' onclick='angular.element(this).scope().startWorkFlowGroupOfSaze(" + cellInfo.data.ID + "," + cellInfo.data.Status + ","+  '"' + cellInfo.data.ContactTitle + '"' + ")'><span class='fa fa-reply'></span>ارسال </button>";
            ...
            $scope.getInvoices = function () {
                ...
                $.ajax({
                    ...
                    url: "/app/api/Contract/GetContracts",
            ...
            $scope.startInvoice = function (id, status, title) {
                ...
                //if (status === 1) {
                //    DevExpress.ui.notify(" این قرارداد قبلا ارسال شده است. ", "warning", 3000);
            ...
            $scope.deleteInvoice = function () {
                ...
                if (items.length === 0) {
                    alertbox({ content: "هیچ قراردادی جهت حذف انتخاب نشده است" });
                    return;
                }
                var s = items.length === 1 ? "قرارداد" : "قراردادهای";
                ...
                if (haveApproved) {
                    ...
                    //tip1 += "نکته: با حذف قراردادهای تایید شده، دریافت ها و پرداخت های ثبت شده برای آنها نیز حذف می گردد.";
                    //tip1 += "</div>";
                    if ($scope.type === 1 || $scope.type === 2) {
                        tip2 = "<div class='bg-warning pd-5 small'>";
                        tip2 += "حذف قرارداد خرید (یا برگشت از فروش) ممکن است باعث ایجاد موجودی منفی در کالاها شود";
                ...
                questionbox({
                    ...
                    onBtn1Click: function () {
                        ...
                        $.ajax({
                            ...
                            url: "/app/api/Contract/DeleteContracts",
                            contentType: "application/json"
                        }).done(function (res) {
                            ...
                            grid.refresh();
                            grid.clearSelection();
                            var strMessage = items.length > 1 ? "قراردادهای " : "قرارداد ";
            ...
            $scope.returnInvoice = function () {
                ...
                if (selectedItems.length === 0) {
                    alertbox({ content: "هیچ قراردادی جهت مرجوع کردن انتخاب نشده است" });
                    return;
                }
                if (selectedItems.length > 1) {
                    alertbox({ content: "برای مرجوع کردن فقط یک قرارداد را انتخاب کنید" });
                    return;
                }
                if (selectedItems[0].Returned) {
                    alertbox({ content: "این قرارداد قبلاً مرجوع شده است." });
                    return;
                }
                questionbox({
                    content: "آیا از مرجوع کردن قرارداد انتخاب شده مطمئن هستید؟",
                    onBtn1Click: function () {
                        if ($scope.type === 0)
            ...
            $scope.markAs = function (status) {
                ...
                var selectedItems = grid.getSelectedRowsData();
                if (selectedItems.length === 0) {
                    alertbox({ content: "هیچ قراردادی انتخاب نشده است" });
                    return;
                }
                var s = selectedItems.length === 1 ? "قرارداد" : "قراردادهای";
                for (var i = 0; i < selectedItems.length; i++) {
                    if (selectedItems[i].Status < 2) {
                        alertbox({ content: "یکی از قراردادهای انتخاب شده تایید نشده است و امکان تغییر وضعیت آن وجود ندارد." });
                        return;
                    }
                }
                if ($scope.calling) return;
            ...
            $scope.printAddressLabelDialog = function () {
                ...
                messagebox({
                    ...
                    "<label><input type='checkbox' id='invoiceNumber' checked> شماره قرارداد</label><br/>" +
                    "<label><input type='checkbox' id='invoiceNote'> افزودن توضیحات قرارداد به برچسب</label>" +
                    "</div>" +
                    "</div><hr>" +
            ...
            $scope.printInvoices = function () {
                var gridSelectedItems = grid.getSelectedRowsData();
                if (gridSelectedItems.length === 0) {
                    alertbox({ content: "ابتدا یک یا چند قرارداد را انتخاب کنید." });
                    return;
                }
                $("#printBtns").hide();
                $("#progressBarDiv").show();
                ...
                (function loop() {
                    callws(DefaultUrl.MainWebService + 'Load_Invoice_TransObj', { id: gridSelectedItems[i].Id })
                        .success(function (result) {
                            ...
                            var l = $scope.invoice.InvoiceItems.length; // محاسبه مالیات و تخفیف کل قرارداد جهت نمایش
                            ...
                            $("#progressPdfTitle").text("در حال تهیه PDF... قرارداد " + Hesabfa.farsiDigit(i + 1) + " از " + Hesabfa.farsiDigit(listCount) + " (" + Hesabfa.farsiDigit(progressValue) + '%)');
                            i++;
