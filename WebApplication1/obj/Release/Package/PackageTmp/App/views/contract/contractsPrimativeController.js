define(['application', 'dataService', 'digitGroup', 'number', 'dx', 'roweditor', 'helper', 'gridHelper', 'messageService'], function (app) {
    app.register.controller('contractsPrimativeController', ['$scope', 'dataService', '$location', '$stateParams', '$compile', '$state', '$rootScope', 'messageService',
        function ($scope, dataService, $location, $stateParams, $compile, $state, $rootScope, messageService) {
            ...
            $rootScope.applicationModule = "پیش قراردادها";
            $scope.applicationDescription = "در این صفحه امکان جستجو، اضافه و ویرایش پیش قراردادها وجود دارد.";
            ...
            $scope.init = function () {
                ...
                if (status && status === 'draft' && invoiceId) {
                    $scope.alert = true;
                    $scope.alertMessage = "پیش قرارداد {2} ذخیره شد - {0} - مبلغ کل: {1}".
                    ...
                    $scope.alert = true;
                    $scope.alertMessage = "پیش قرارداد جهت تایید ثبت و ذخیره شد - {0} - مبلغ کل: {1}".
                    ...
                    $scope.alert = true;
                    $scope.alertMessage = "پرداخت به مبلغ {0} ثبت شد - {1} - سر رسید: {2} - پرداخت در {3} - مبلغ پیش قرارداد: {4} - باقیمانده: {5}".
                    ...
                    $scope.alert = true;
                    $scope.alertMessage = "پیش قرارداد شماره {0} از سیستم حذف شد.".
                ...
                if ($scope.type === 0) $rootScope.pageTitle("پیش قراردادهای فروش");
                else if ($scope.type === 1) $rootScope.pageTitle("پیش قراردادهای خرید");
                ...
                var lookup = { dataSource: [{ key: 0, val: "ذخیره موقت" }, { key: 1, val: "ارسال شده" }, { key: 2, val: "تایید پیش قرارداد" } ], valueExpr: 'key', displayExpr: 'val' };
                ...
                grid = dxGrid({
                    ...
                    columns: [
                        ...
                        { caption: 'مبلغ', dataField: 'Payable', dataType: "number", format: "#", width: 100, printWidth: 3, printFormat: "#" },
                        ...
                        //{ caption: 'شهر', dataField: 'City', width: 60, hidingPriority: 0 },
                        {
                            caption: 'عملیات', dataField: '', width: 70, printVisible: false, cellTemplate: function (cellElement, cellInfo) {
                                if (cellInfo.data.Status === 0) {
                                    ...
                                    html = "<a class='text-info p-l-15' href='javascript:void(0);' onclick='angular.element(this).scope().viewInvoice(" + cellInfo.data.ID + ")'><span class='fa fa-eye'></span></a>";
                                    html += "<a class='text-danger' href='javascript:void(0);' onclick='angular.element(this).scope().deleteInvoice()'><span class='fa fa-times'></span></a>";
                                    cellElement.html(html);
                                }
                        ...
                        {
                            caption: 'ارسال', dataField: '', width: 150, printVisible: false, cellTemplate: function (cellElement, cellInfo) {
                                if (cellInfo.data.Status === 0) {

                                    var html = "<button type='button' class='k-button k-button - icontext' onclick='angular.element(this).scope().startInvoice(" + cellInfo.data.ID + "," + cellInfo.data.Status + "," + '"' + cellInfo.data.ContactTitle + '"' +")'><span class='fa fa-reply'></span>ارسال</button>";
                                    cellElement.html(html);
                                }

                                //else {
                                //    html = "<button type='button' class='k-button k-button - icontext' onclick='angular.element(this).scope().startInvoice(" + cellInfo.data.ID + "," + cellInfo.data.Status + ","+'"' + cellInfo.data.ContactTitle +'"'+")'><span class='fa fa-reply'></span> ارسال</button>";
                                //    cellElement.html(html);
                                //}
            ...
            $scope.startInvoice = function (id, status, title) {
                ...
                if (status === 1) {
                    DevExpress.ui.notify(" این پیش قرارداد قبلا ارسال شده است. ", "warning", 3000);
                ...
                $scope.model.Code = 'ContractPrimitive';
                $scope.model.ID = id;
                // var ID = this.dataItem($(e.currentTarget).closest("tr")).ID;
                $scope.model.OrganizationID = 22;//this.dataItem($(e.currentTarget).closest("tr")).OrganizationID;
               // var InstanceTitle = " پیش قرارداد " + title;//this.dataItem($(e.currentTarget).closest("tr")).DaroukhanehNam;
                ...
                dataService.addEntity('/app/api/Contract/StartWorkFlowContractPrimative', $scope.model).then(function (data) {
            ...
            $scope.deleteInvoice = function () {
                ...
                if (items.length === 0) {
                    alertbox({ content: "هیچ پیش قراردادی جهت حذف انتخاب نشده است" });
                    return;
                }
                var s = items.length === 1 ? "پیش قرارداد" : "پیش قراردادهای";
                ...
                if (haveApproved) {
                    ...
                    //tip1 += "نکته: با حذف پیش قراردادهای تایید شده، دریافت ها و پرداخت های ثبت شده برای آنها نیز حذف می گردد.";
                    //tip1 += "</div>";
                    if ($scope.type === 1 || $scope.type === 2) {
                        tip2 = "<div class='bg-warning pd-5 small'>";
                        tip2 += "حذف پیش قرارداد خرید (یا برگشت از فروش) ممکن است باعث ایجاد موجودی منفی در کالاها شود";
                ...
                questionbox({
                    ...
                    onBtn1Click: function () {
                        ...
                        $.ajax({
                            ...
                            grid.refresh();
                            grid.clearSelection();
                            var strMessage = items.length > 1 ? "پیش قراردادهای " : "پیش قرارداد ";
            ...
            $scope.returnInvoice = function () {
                ...
                if (selectedItems.length === 0) {
                    alertbox({ content: "هیچ پیش قراردادی جهت مرجوع کردن انتخاب نشده است" });
                    return;
                }
                if (selectedItems.length > 1) {
                    alertbox({ content: "برای مرجوع کردن فقط یک پیش قرارداد را انتخاب کنید" });
                    return;
                }
                if (selectedItems[0].Returned) {
                    alertbox({ content: "این پیش قرارداد قبلاً مرجوع شده است." });
                    return;
                }
                questionbox({
                    content: "آیا از مرجوع کردن پیش قرارداد انتخاب شده مطمئن هستید؟",
                    onBtn1Click: function () {
                        if ($scope.type === 0)
            ...
            $scope.markAs = function (status) {
                ...
                var selectedItems = grid.getSelectedRowsData();
                if (selectedItems.length === 0) {
                    alertbox({ content: "هیچ پیش قراردادی انتخاب نشده است" });
                    return;
                }
                var s = selectedItems.length === 1 ? "پیش قرارداد" : "پیش قراردادهای";
                for (var i = 0; i < selectedItems.length; i++) {
                    if (selectedItems[i].Status < 2) {
                        alertbox({ content: "یکی از پیش قراردادهای انتخاب شده تایید نشده است و امکان تغییر وضعیت آن وجود ندارد." });
                        return;
                    }
                }
                if ($scope.calling) return;
            ...
            $scope.printAddressLabelDialog = function () {
                ...
                messagebox({
                    ...
                    "<label><input type='checkbox' id='invoiceNumber' checked> شماره پیش قرارداد</label><br/>" +
                    "<label><input type='checkbox' id='invoiceNote'> افزودن توضیحات پیش قرارداد به برچسب</label>" +
                    "</div>" +
                    "</div><hr>" +
            ...
            $scope.printInvoices = function () {
                var gridSelectedItems = grid.getSelectedRowsData();
                if (gridSelectedItems.length === 0) {
                    alertbox({ content: "ابتدا یک یا چند پیش قرارداد را انتخاب کنید." });
                    return;
                }
                $("#printBtns").hide();
                $("#progressBarDiv").show();
                ...
                (function loop() {
                    callws(DefaultUrl.MainWebService + 'Load_Invoice_TransObj', { id: gridSelectedItems[i].Id })
                        .success(function (result) {
                            ...
                            var l = $scope.invoice.InvoiceItems.length; // محاسبه مالیات و تخفیف کل پیش قرارداد جهت نمایش
                            ...
                            $("#progressPdfTitle").text("در حال تهیه PDF... پیش قرارداد " + Hesabfa.farsiDigit(i + 1) + " از " + Hesabfa.farsiDigit(listCount) + " (" + Hesabfa.farsiDigit(progressValue) + '%)');
                            i++;
