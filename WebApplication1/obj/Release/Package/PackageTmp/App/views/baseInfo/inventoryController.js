define(['application', 'combo', 'scrollbar', 'helper', 'editItem', 'editContact', 'nodeSelect', 'roweditor', 'gridHelper','dx'], function (app) {
    app.register.controller('inventoryController', ['$scope','$rootScope', '$location', '$compile','$state',
        function ($scope, $rootScope, $location, $compile,$state) {
	var grid;
	$scope.init = function () {
		$rootScope.pageTitle("کالاها و خدمات");
		$('#businessNav').show();
		$scope.type = "all";
		$scope.nodeId = 0;
        ...
        grid = dxGrid({
			...
			columns: [
				...
				{
					caption: 'نام کالا یا خدمات', printWidth: 7, dataField: 'Name', allowEditing: false, cellTemplate: function (cellElement, cellInfo) {
	...
	$scope.getItems = function () {
		...
		$.ajax({
		    ...
		    contentType: "application/json"
                ...
                alertbox({ content: error.responseJSON.data, type: "error" });
            });
	};
	...
	$scope.deleteItems = function () {
		...
		function deleteItems(items, startIndex, count, successCallback, errorCallback) {
			...
			$.ajax({
			    ...
			    contentType: "application/json"
                    ...
                    errorCallback(error.responseJSON.data);
		...
		function startDelete(items, start, count) {
			if (start >= items.length) {
				...
				$scope.calling = false;
				var strMessage = deletedCount > 1 ? "کالاها و خدمات " : "کالا یا خدمات ";
		...
		questionbox({
			content: "آيا از حذف کالاهای انتخاب شده مطمئن هستيد؟",
