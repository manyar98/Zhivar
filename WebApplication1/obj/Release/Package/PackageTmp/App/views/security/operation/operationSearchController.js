define(['application', 'dataService', 'messageService', 'kendoGridFa'], function (app) {
    app.register.controller('operationSearchController', ['$scope', '$rootScope', 'dataService', 'messageService', '$state',
        function ($scope, $rootScope, dataService, messageService, $state) {

            $rootScope.applicationModule = "مدیریت دسترسی ها";
            $scope.applicationDescription = "در این صفحه امکان جستجو دسترسی های تعریف شده، اضافه کردن دسترسی جدید، ویرایش و حذف دسترسی وجود دارد."
            ...
            $scope.add = function () {
                $state.go("operationInfo", { id: 0, mode: "-1" });
            }

            $scope.addParent = function (e) {
                var id = this.dataItem($(e.currentTarget).closest("tr")).ID;
                $state.go("operationInfo", { id: id, mode: "-1" });
            }
            ...
            function loadTamplate() {
                return '<button type="button" class="k-button k-button-icontext" ng-click="add()"><span class="k-icon k-add"></span>اضافه کردن دسترسی به ریشه</button>';
            }
            ...
            $scope.deleteRow = function () {
                ...
                dataService.deleteEntity("/app/api/Operation/", $scope.confirmDelete.model.ID).then(function (data) {
